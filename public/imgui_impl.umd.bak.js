(function (global, factory) {
	...
	function canvas_on_keydown(event) {
		...
		ImGui__namespace.ASSERT(
			key_index >= 0 && key_index < ImGui__namespace.ARRAYSIZE(io.KeysDown)
		);
	...
	function Init(value) {
		...
		if (typeof window !== "undefined") {
			...
			ImGui__namespace.LoadIniSettingsFromMemory(
		...
		io.KeyMap[ImGui__namespace.Key.KeyPadEnter] =
			key_code_to_index["NumpadEnter"];
		...
		if (io.WantSaveIniSettings) {
			...
			if (typeof window !== "undefined") {
				window.localStorage.setItem(
					"imgui.ini",
					ImGui__namespace.SaveIniSettingsToMemory()
				);
			}
		}
	...
	function RenderDrawData(draw_data = ImGui__namespace.GetDrawData()) {
		...
		exports.gl &&
			exports.gl.vertexAttribPointer(
				...
				exports.gl.FLOAT,
				false,
				ImGui__namespace.DrawVertSize,
				ImGui__namespace.DrawVertPosOffset
			);
		exports.gl &&
			exports.gl.vertexAttribPointer(
				g_AttribLocationUV,
				...
				false,
				ImGui__namespace.DrawVertSize,
				ImGui__namespace.DrawVertUVOffset
			);
		exports.gl &&
			exports.gl.vertexAttribPointer(
				...
				true,
				ImGui__namespace.DrawVertSize,
				ImGui__namespace.DrawVertColOffset
			);
		// Draw
		const pos = draw_data.DisplayPos;
		const idx_buffer_type =
			(exports.gl &&
				(ImGui__namespace.DrawIdxSize === 4
			...
			draw_list.IterateDrawCmds((draw_cmd) => {
				...
				if (!exports.gl && !exports.ctx) {
					...
					for (let i = 0; i < Math.min(3, draw_cmd.ElemCount); ++i) {
						const view = new ImGui__namespace.DrawVert(
							draw_list.VtxBuffer.buffer,
							draw_list.VtxBuffer.byteOffset + i * ImGui__namespace.DrawVertSize
						);
						console.log(
				...
				if (draw_cmd.UserCallback !== null) {
					...
					const clip_rect = new ImGui__namespace.Vec4(
					...
					if (
						...
						exports.gl &&
							exports.gl.drawElements(
								...
								draw_cmd.ElemCount,
								idx_buffer_type,
								draw_cmd.IdxOffset * ImGui__namespace.DrawIdxSize
							);
						if (exports.ctx) {
							exports.ctx.save();
							...
							const idx =
								ImGui__namespace.DrawIdxSize === 4
									? new Uint32Array(
											...
											draw_list.IdxBuffer.byteOffset +
												draw_cmd.IdxOffset * ImGui__namespace.DrawIdxSize
									  )
									: new Uint16Array(
											draw_list.IdxBuffer.buffer,
							...
							for (let i = 0; i < draw_cmd.ElemCount; i += 3) {
								...
								const i1 = idx[i + 1];
								const i2 = idx[i + 2];
								const v0 = new ImGui__namespace.DrawVert(
									draw_list.VtxBuffer.buffer,
									draw_list.VtxBuffer.byteOffset +
										i0 * ImGui__namespace.DrawVertSize
								);
								const v1 = new ImGui__namespace.DrawVert(
									draw_list.VtxBuffer.buffer,
									draw_list.VtxBuffer.byteOffset +
										i1 * ImGui__namespace.DrawVertSize
								);
								const v2 = new ImGui__namespace.DrawVert(
									draw_list.VtxBuffer.buffer,
									draw_list.VtxBuffer.byteOffset +
										i2 * ImGui__namespace.DrawVertSize
								);
								const i3 = idx[i + 3];
								const i4 = idx[i + 4];
								const i5 = idx[i + 5];
								const v3 = new ImGui__namespace.DrawVert(
									draw_list.VtxBuffer.buffer,
									draw_list.VtxBuffer.byteOffset +
										i3 * ImGui__namespace.DrawVertSize
								);
								const v4 = new ImGui__namespace.DrawVert(
									draw_list.VtxBuffer.buffer,
									draw_list.VtxBuffer.byteOffset +
										i4 * ImGui__namespace.DrawVertSize
								);
								const v5 = new ImGui__namespace.DrawVert(
									draw_list.VtxBuffer.buffer,
									draw_list.VtxBuffer.byteOffset +
										i5 * ImGui__namespace.DrawVertSize
								);
								let quad = true;
								let minmin = v0;
								let minmax = v0;
